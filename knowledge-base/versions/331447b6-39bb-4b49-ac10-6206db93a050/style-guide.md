...
`钱玮` 版本整体呈现解释性、讨论型、学习型翻译风格。译文经常不仅给出中文结果，还保留术语线索、补出语法关系，并在必要处显示译者对原文结构的理解过程。这个版本适合研究“学习共同体如何处理巴利注释文本”，也适合作为 LLM 识别翻译决策、术语保留和解释性增补的语料。

从全量 `6350` 条记录看，该版本巴利原文平均长度为 `86.42` 个字符，中文译文平均长度为 `40.27` 个字符。长度关系只能说明整体压缩或扩展趋势，不能直接判断翻译质量；真正有分析价值的是术语保留、方括号补足、换行拆解、定义公式和并列解释等可证据化信号。

## 可量化信号

| Dimension | Observation | Count |
| --- | --- | --- |
| pali_retention | 译文经常用括号保留巴利词形。 | 1411 |
| translator_supplement | 译文使用方括号补出隐含或解释性内容。 | 1288 |
| line_break_decomposition | 译文使用换行拆分并列解释或复杂词源分析。 | 94 |

## 巴利词形保留

全量扫描发现 `1411` 条译文含有括号中的巴利形式，并抽取出 `1756` 个术语候选。这类处理常见于三种情况：
...
该版本有 `1288` 条译文含方括号。方括号通常表示译者补足、解释、不确定判断或语法成分显化。
...
该版本有 `94` 条译文含换行。换行多用于拆分复杂解释，尤其是词源说明、多个 `vā` 并列解释、长句中的义项分解或讨论式补注。这种换行不是纯排版噪声，而可能表示译者对并列解释层次的理解。
...
这种风格适合建立版本内术语表，但不宜直接推出跨版本标准译法。不同版本可能更偏流畅汉语、传统佛典译语或现代解释语；本版本的术语记录应先作为 `钱玮` 版本用词习惯保存。

## 句法风格

译者经常把巴利句法关系转成显性的中文关系词。例如工具格、原因关系、处所关系、定义关系、并列解释等，常通过“以、由于、在、即、所谓、或”等表达。该版本尤其适合分析注释书中的定义句、词源句和并列解释句。

不过，形式信号不能直接等同于翻译规则。例如 `-tvā/-tvāna` 绝对分词候选有 `698` 条，但很多译文未必明确译成“之后、先、以……方式”。因此这类条目目前只能作为候选集合，需要人工筛选后再总结具体翻译技巧。
